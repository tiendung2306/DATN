...
Dưới góc nhìn của hệ phân tán, quá trình tiến hóa trạng thái của một nhóm MLS mang bản chất của một Máy trạng thái nhân bản (\textit{State Machine Replication} - SMR). Trong mô hình này, mỗi thiết bị của người dùng duy trì một bản sao của trạng thái mật mã (như Ratchet Tree, KeySchedule). Các thao tác \textit{Commit} chính là các phép chuyển trạng thái (\textit{state transition}). Đặc điểm làm cho bài toán đồng thuận trong MLS trở nên khắt khe là tính chất hoàn toàn không giao hoán (\textit{strictly non-commutative}) của thao tác Commit. Nếu hai thành viên cùng tạo ra Commit tại cùng một epoch, việc áp dụng Commit thứ nhất rồi đến Commit thứ hai sẽ sinh ra một bộ khóa hoàn toàn khác biệt so với việc áp dụng theo thứ tự ngược lại. Nó không thể được tự động dung hòa bằng các thuật toán gộp dữ liệu bình thường, bởi mỗi bước chuyển trạng thái mật mã đều dùng hàm băm và dẫn xuất ra khóa một chiều dựa trên chính trạng thái liền trước đó. Do tính chất không giao hoán này, hệ thống bắt buộc phải duy trì một thứ tự toàn cục (\textit{total order}) để tránh hiện tượng rẽ nhánh.
