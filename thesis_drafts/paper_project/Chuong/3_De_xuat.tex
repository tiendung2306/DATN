...
Trọng tâm của chương này là phương pháp đề xuất ở mức giao thức. Cụ thể, chương lần lượt trình bày bài toán và các yêu cầu thiết kế, cơ chế người ghi duy nhất theo epoch, cơ chế kiểm tra nhất quán epoch, cơ chế phát hiện và hàn gắn phân nhánh, cùng cách dùng đồng hồ logic lai để sắp xếp thông điệp ứng dụng. Ở cuối chương, đồ án chỉ dành một mục ngắn để nêu cách ứng dụng thử nghiệm hiện thực các nguyên tắc đó; phần kiểm chứng bằng ứng dụng thử nghiệm và ảnh minh họa được chuyển sang Chương 5 và phụ lục.
...
Vì vậy, phương pháp đề xuất trong đồ án phải đồng thời đáp ứng bốn yêu cầu: (i) giảm mạnh khả năng xuất hiện concurrent commits khi mạng còn liên thông; (ii) buộc mỗi nút chỉ xử lý thông điệp trên trạng thái MLS tương thích với epoch cục bộ của nó; (iii) phát hiện được sự phân kỳ trạng thái khi network partition xảy ra và hỗ trợ hội tụ trở lại một nhánh hợp lệ; (iv) giữ được một thứ tự hiển thị ổn định cho các thông điệp ứng dụng trong cùng một Epoch mà không nhầm lẫn vai trò đó với thứ tự Commit.

\subsection{Kiến trúc logic của giải pháp}

Ở mức logic, phương pháp được tổ chức thành hai tầng trách nhiệm. Tầng thứ nhất là tầng điều phối, chịu trách nhiệm quyết định khi nào một nút được quyền tạo Commit, cách phân loại thông điệp theo epoch, cách đối chiếu trạng thái nhánh và cách khôi phục sau phân mảnh mạng. Tầng thứ hai là tầng MLS, chịu trách nhiệm xử lý Proposal, Commit, Welcome, External Join, cũng như mã hóa và giải mã thông điệp theo đúng quy tắc của MLS \cite{rfc9420}.
...
Phương pháp đề xuất gồm bốn cơ chế gắn chặt với nhau. Cơ chế thứ nhất là người ghi duy nhất theo epoch, nhằm giới hạn quyền phát hành Commit. Cơ chế thứ hai là kiểm tra nhất quán epoch, nhằm ngăn nút xử lý thông điệp trên một trạng thái MLS không tương thích. Cơ chế thứ ba là phát hiện và hàn gắn phân nhánh, nhằm đưa các nút hội tụ trở lại sau network partition. Cơ chế cuối cùng là đồng hồ logic lai HLC, dùng để sắp xếp thông điệp ứng dụng sau khi các thông điệp đó đã đi qua lớp kiểm tra và xử lý của MLS.
...
Nguyên nhân trực tiếp của phân nhánh trạng thái trong MLS trên P2P là việc nhiều Commit hợp lệ cùng xuất hiện tại một epoch. Một hướng tiếp cận là chấp nhận cho nhiều Commit xuất hiện rồi xử lý xung đột về sau. Tuy nhiên, cách đó làm tăng độ phức tạp ở tầng trạng thái và khiến hệ thống phải đối mặt với nhiều trạng thái tạm thời hơn \cite{alwen2023fork,kohbrok2025dmls}. Đồ án chọn hướng ngược lại: cố gắng hạn chế xung đột ngay từ đầu bằng cách chỉ cho phép một thành viên phát hành Commit trong mỗi epoch.
...
Ở đây, $H$ là hàm băm mật mã như SHA-256. Việc đưa $group\_id$ vào đầu vào giúp một nút không mặc nhiên giữ ưu thế ở mọi nhóm; việc đưa $E$ vào đầu vào giúp quyền giữ Token thay đổi theo từng epoch; còn định danh nút bảo đảm mọi nút đang có cùng góc nhìn về tập ứng viên sẽ đi tới cùng kết quả. Thuật toán bầu chọn được mô tả trong Thuật toán~\ref{alg:compute_token_holder}.
...
Ưu điểm chính của cách chọn này là không cần thêm một vòng bỏ phiếu mạng cho mỗi Commit. Nếu các nút có cùng trạng thái nhóm và cùng góc nhìn về tập ứng viên, chúng sẽ tự tính ra cùng một Token Holder. Chi phí của bước bầu chọn vì thế chủ yếu là chi phí cục bộ, tăng tuyến tính theo số lượng ứng viên hợp lệ.
...
Trong mạng P2P, Token Holder có thể ngoại tuyến hoặc không kịp phát hành Commit. Để tránh bế tắc, các nút duy trì góc nhìn hoạt động cục bộ của nhóm. Nếu Token Holder hiện tại không còn được quan sát là đang hoạt động hoặc vượt quá thời gian chờ hợp lý, các nút sẽ loại nó khỏi tập ứng viên của Epoch đó và tính lại Token Holder.

Cần nhấn mạnh rằng failover này không phải là một cơ chế đồng thuận Byzantine đầy đủ. Khi network partition xảy ra, các phân vùng khác nhau có thể có các $ActiveView$ khác nhau, dẫn tới các Token Holder khác nhau. Do đó, vai trò đúng của Single-Writer là giảm mạnh nguy cơ concurrent commits khi các nút còn có cùng góc nhìn, chứ không phải xóa bỏ hoàn toàn mọi khả năng fork trong mọi điều kiện mạng.
...
Single-Writer hoạt động tốt nhất khi các nút còn có cùng góc nhìn về nhóm. Tuy nhiên, trong mạng P2P, network partition có thể chia nhóm thành nhiều phân vùng không liên lạc được với nhau. Khi đó, mỗi phân vùng chỉ quan sát được một phần thành viên đang hoạt động và có thể tính ra các Token Holder khác nhau. Nếu các phân vùng tiếp tục tiến hóa độc lập, hệ thống có thể xuất hiện nhiều nhánh trạng thái MLS khác nhau.
...
\subsection{Quy trình hàn gắn phân nhánh}
...
\caption{Quy trình hàn gắn phân nhánh ($ForkHealing$)}
...
Kết chương. Chương 3 đã trình bày lớp điều phối phi tập trung mà đồ án đề xuất cho MLS trên mạng ngang hàng. Phần cốt lõi của phương pháp gồm bốn cơ chế: người ghi duy nhất theo epoch để hạn chế concurrent commits, kiểm tra nhất quán epoch để bảo vệ trạng thái cục bộ, phát hiện và hàn gắn phân nhánh để hỗ trợ hội tụ sau network partition, và HLC để sắp thứ tự thông điệp ứng dụng.
