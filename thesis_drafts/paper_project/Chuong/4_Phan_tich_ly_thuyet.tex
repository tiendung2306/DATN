...
Điều cần nhấn mạnh là bất biến trên chỉ đúng khi các nút đang có cùng góc nhìn về nhóm. Nếu hai phân vùng giữ hai $ActiveView$ khác nhau, chúng có thể hình thành hai tập $Eligible(E)$ khác nhau và từ đó chọn ra hai Token Holder khác nhau. Vì vậy, Single-Writer không phải là lời hứa rằng hệ thống không bao giờ phân nhánh; vai trò đúng của nó là giảm mạnh khả năng sinh concurrent commits khi mạng còn đủ liên thông để các nút chia sẻ cùng một view.

\section{Bất biến 3: Dấu hiệu phân nhánh phải quan sát được trong phạm vi giả định}
\label{sec:bat_bien_fork_detection}

\subsection{Phân biệt chậm đồng bộ và phân nhánh thực sự}

Trong mạng ngang hàng, không phải mọi khác biệt quan sát được đều là phân nhánh. Nếu một nút có epoch thấp hơn nút khác, khả năng cao hơn là nó đang chậm đồng bộ. Ngay cả khi hai nút cùng mang một số epoch, khác biệt tạm thời về dữ liệu quan sát được vẫn cần được đối chiếu cẩn thận trước khi kết luận có fork. Vì vậy, quá trình phát hiện phân nhánh trong đồ án được đặt trên ba bước liên tiếp: nhận diện dấu hiệu nghi ngờ, đối chiếu trạng thái và thử bắt kịp đồng bộ, rồi mới kết luận cần hàn gắn nếu khác biệt vẫn còn tồn tại.

\subsection{Fingerprint trạng thái}

Để phục vụ bước đối chiếu, các nút công bố định kỳ một bản tóm tắt trạng thái ngắn gọn:
...
Trong đó, $treeHash$ phản ánh trạng thái cây MLS hiện tại, còn $commitHash$ là giá trị băm của Commit gần nhất mà lớp điều phối gắn với nhánh đang xét. Bản tóm tắt này không thay thế trạng thái MLS đầy đủ, nhưng đủ để làm tín hiệu so sánh giữa các nút.

\textbf{Mệnh đề 3.} Trong phạm vi giả định của đồ án, nếu hai nút đã trao đổi đủ thông tin so sánh mà vẫn công bố cùng $groupID$, cùng epoch nhưng khác $treeHash$ hoặc khác $commitHash$, thì lớp điều phối có cơ sở để kết luận rằng chúng không còn đứng trên cùng một nhánh trạng thái.

\textit{Lập luận.} Khi chỉ có chậm đồng bộ đơn thuần, sự khác biệt thường xuất hiện dưới dạng một nút đi sau về epoch hoặc thiếu một đoạn lịch sử gần đây. Trong trường hợp đó, bước đồng bộ sẽ giúp nút đi sau bắt kịp và hai phía quay về cùng trạng thái. Ngược lại, nếu sau bước đối chiếu mà hai nút vẫn giữ cùng epoch nhưng khác $treeHash$ hoặc khác $commitHash$, thì khác biệt đó không còn được giải thích bằng việc một phía đơn thuần nhận tin chậm. Nó cho thấy hai bên đã đi qua hai chuỗi Commit khác nhau hoặc đã duy trì hai trạng thái cây khác nhau ở cùng một mức tiến hóa logic.

Lập luận trên cố ý không xem một announcement đơn lẻ là bằng chứng tuyệt đối. Giá trị của nó nằm ở chỗ tạo ra tín hiệu đủ mạnh để lớp điều phối biết khi nào cần chuyển từ chế độ bắt kịp đồng bộ sang chế độ phục hồi phân nhánh.
...
Các thao tác thêm thành viên, loại bỏ thành viên hoặc cập nhật khóa đều đi qua Proposal và Commit. Ở mức TreeKEM, việc tổ chức khóa theo cây giúp nhiều thao tác hiệu quả hơn so với cách mã hóa lại riêng cho từng cặp thành viên \cite{rfc9420}. Vì vậy, khi quy mô nhóm tăng, chi phí thay đổi thành viên vẫn tăng, nhưng không tăng theo cách ngây thơ như mô hình quảng bá từng cặp.

\subsection{Chi phí của Single-Writer}

Single-Writer không yêu cầu một vòng đồng thuận mạng riêng để bầu Token Holder trong điều kiện các nút có cùng view. Mỗi nút chỉ cần duyệt qua tập $Eligible(E)$ và tính hàm băm cho từng ứng viên. Nếu số ứng viên hợp lệ là $N$, chi phí tính toán cục bộ của bước chọn Token Holder là $O(N)$ theo cách cài đặt trực tiếp.
...
Đối chiếu trạng thái để nhận biết phân nhánh đòi hỏi các nút trao đổi một bản tóm tắt ngắn chứa số epoch, $treeHash$ và $commitHash$. Kích thước của bản tóm tắt là hằng số theo kích thước nhóm, nên chi phí của một lần công bố trạng thái đơn lẻ cũng ở mức $O(1)$ theo số trường cần gửi. Ở quy mô toàn hệ thống, chi phí mạng của cơ chế này phụ thuộc thêm vào tần suất công bố và số kết nối mà mỗi nút duy trì.
...
\begin{table}[htbp]
	...
	\begin{tabular}{|p{4.0cm}|p{5.0cm}|p{5.0cm}|}
		...
		\hline
		Một Token Holder trong cùng view & Single-Writer theo epoch & Giảm concurrent commits khi các nút có cùng góc nhìn về nhóm. \\
		\hline
		Dấu hiệu phân nhánh có thể quan sát được & Bản tóm tắt trạng thái, $treeHash$, $commitHash$ & Cho phép phân biệt giữa chậm đồng bộ và phân nhánh thực sự sau bước đối chiếu. \\
		\hline
...
Single-Writer phát huy hiệu quả rõ nhất khi các nút còn chia sẻ cùng một view về nhóm. Khi góc nhìn giữa các phân vùng đã lệch nhau, chính các cơ chế đối chiếu trạng thái và phục hồi phân nhánh mới trở thành phần quyết định khả năng hội tụ. Tương tự, quy tắc chọn nhánh không bảo đảm rằng mọi nút sẽ chọn cùng một kết quả ngay tức thì trong mọi thời điểm; để điều đó xảy ra, các nút phải có đủ thông tin về các nhánh đang tồn tại.
...
Kết chương. Chương này đã phân tích cơ sở lý thuyết của lớp điều phối phi tập trung được đề xuất cho MLS trên mạng ngang hàng. Các lập luận cho thấy, trong phạm vi giả định của đồ án, phương pháp có thể bảo toàn được các bất biến quan trọng như việc epoch cục bộ chỉ giữ nguyên hoặc tăng, tính đơn trị của Token Holder trong cùng view, khả năng nhận biết phân nhánh sau bước đối chiếu trạng thái, nguyên tắc không gộp trực tiếp hai nhánh MLS đã tách và vai trò giới hạn của HLC ở tầng ứng dụng.
