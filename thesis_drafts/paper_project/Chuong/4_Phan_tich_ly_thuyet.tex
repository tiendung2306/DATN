...
\subsection{External Join như cơ chế hội tụ}
...
\textbf{Mệnh đề 4.} Nếu các nút quan sát cùng một tập nhánh và cùng áp dụng quy tắc chọn nhánh ở Công thức~\eqref{eq:branch_weight_analysis}, thì chúng sẽ có cùng cơ sở để chọn nhánh tiếp tục; việc đưa các nút của nhánh thua gia nhập lại qua thao tác hợp lệ của MLS giúp hệ thống hội tụ mà không phải tự chế ra một trạng thái mật mã mới.
...
Mục này trình bày sự đối sánh về mặt lý thuyết giữa giao thức điều phối đề xuất (Single-Writer Token kết hợp Autonomous Replay) với các cơ chế đồng thuận truyền thống (như Raft hay BFT), nhằm làm rõ tính phù hợp của giải pháp đối với đặc thù bất đồng bộ của mạng ngang hàng.
...
Thứ hai, về khả năng hoạt động khi mạng bị chia cắt (Partition Tolerance). Trong kịch bản mạng bị chia cắt thành các phân vùng không liên thông, thuật toán Raft hoặc Paxos yêu cầu một phân vùng phải chứa phần lớn số nút ($> N/2$) để có thể tiếp tục xác nhận trạng thái mới. Phân vùng nhỏ hơn sẽ hoàn toàn mất khả năng tiến hóa trạng thái (mất Availability). Ngược lại, thiết kế của đồ án đi theo hướng ưu tiên tính Khả dụng (Availability) trong định lý CAP. Nếu Token Holder nằm ở một phân vùng, phân vùng đó vẫn có thể vận hành và cập nhật trạng thái MLS một cách độc lập. Những nút ở phân vùng bị đứt kết nối sẽ tạm thời không đồng bộ, nhưng khi mạng được khôi phục, chúng sẽ sử dụng quy tắc chọn nhánh và cơ chế External Join để hội tụ trở lại trạng thái hợp lệ, qua đó đạt được tính Nhất quán cuối (Eventual Consistency). Sự đánh đổi này đặc biệt phù hợp cho các ứng dụng trò chuyện phi tập trung, nơi yêu cầu không làm gián đoạn trải nghiệm người dùng khi rớt mạng được đặt lên hàng đầu.
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3.5cm}|p{3.5cm}|p{3.5cm}|p{4.5cm}|}
		...
		\hline
		\textbf{Mô hình nhất quán} & Nhất quán mạnh (Strong) & Nhất quán mạnh (Strong) & \textbf{Nhất quán cuối (Eventual Consistency)} \\
		\hline
