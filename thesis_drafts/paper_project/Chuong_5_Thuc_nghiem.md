...
| `evaluation/data/mls_optimization_benchmark.csv` | Kết quả benchmark pairwise và full-blob MLS |
...
| MLS benchmark | Rust benchmark | Chi phí pairwise/full-blob MLS | So sánh xu hướng chi phí crypto theo $N$ | Không đại diện trực tiếp cho UX end-to-end |
...
Đối với các kết quả latency, luận văn không nên diễn giải từng con số đơn lẻ như hằng số tuyệt đối. Cách đọc phù hợp là so sánh xu hướng: đường pairwise tăng tuyến tính rõ, còn đường full-blob MLS chịu thêm overhead theo kích thước state do phải import/export toàn bộ `group_state` qua sidecar.
...
Đường `pairwise_hash_sanity_not_e2ee` chỉ là sanity check CPU/hash, không nên dùng làm baseline E2EE trong luận văn. Các đường thử nghiệm nội bộ không thuộc production path của ứng dụng không được dùng làm kết quả chính trong luận văn.
...
Benchmark MLS optimization cho thấy baseline pairwise tăng tuyến tính rõ rệt theo số thành viên. Ở $N=4096$, median của `pairwise_baseline` là khoảng 955 ms, trong khi `current_full_blob_mls_encrypt` khoảng 79 ms. Kết quả này cho thấy MLS giảm đáng kể chi phí phía sender so với pairwise E2EE, nhưng đường production full-blob vẫn chịu overhead theo kích thước `GroupState`.

Một số điểm dữ liệu chính:

| N | Pairwise median (ms) | Full-blob MLS encrypt (ms) |
|---:|---:|---:|
| 16 | 3,62 | 0,55 |
| 256 | 59,62 | 5,41 |
| 1024 | 238,75 | 19,86 |
...
- Không nên dùng kết quả thử nghiệm không thuộc production path để kết luận về hiệu năng của app chính.
...
2. Hiệu năng IPC và persistence của đường sidecar full-blob chưa được tối ưu sâu; đây là hướng phát triển tương lai nếu triển khai ở quy mô nhóm rất lớn.
