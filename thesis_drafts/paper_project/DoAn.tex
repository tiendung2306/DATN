...
\usepackage[backend=bibtex,style=ieee,language=english]{biblatex}  %backend=biber is 'better'

\DefineBibliographyStrings{english}{
  url = {[Online]. Available:\space},
  urlseen = {visited on},
  jourser = {Ser.},
  in = {in},
  pages = {pp.},
...
\def \TITLE{Nghiên cứu và ứng dụng giao thức điều phối phi tập trung cho MLS trên mạng ngang hàng}
\def \AUTHOR{Lê Tiến Dũng}
...
\newcommand{\AppDemoPlaceholder}[2]{%
    \fbox{%
        \parbox[c][0.22\textheight][c]{0.86\textwidth}{%
            \centering
            \textbf{Ảnh chụp ứng dụng sẽ được bổ sung sau}\\[0.5em]
            #2\\[0.75em]
            \path{#1}%
        }%
    }%
...
\newcommand{\AppDemoGraphic}[3][0.9\textwidth]{%
    \IfFileExists{#2}{%
        \includegraphics[width=#1]{#2}%
    }{%
        \AppDemoPlaceholder{#2}{#3}%
    }%
}

\newcommand{\AppDemoSubfigure}[4][0.31\textwidth]{%
    \begin{subfigure}[t]{#1}
        \centering
        \IfFileExists{#2}{%
            ...
            \fbox{%
                \parbox[c][0.19\textheight][c]{0.9\textwidth}{%
                    \centering
                    #4\\[0.6em]
                    \path{#2}%
                }%
...
\fancyfoot[RE, LO]{\thepage} %RO=right odd, RE=right even
...
\newpage
\chapter{ỨNG DỤNG DEMO VÀ KỊCH BẢN MINH HỌA}
\label{appendix:app_demo}
\subfile{Chuong/Phu_luc_Ung_dung_demo}

\newpage
\chapter{HÀM TRỌNG SỐ NHÁNH}
\label{appendix:branch_weight}
