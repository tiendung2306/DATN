...
<#
.SYNOPSIS
  Chạy instance thứ tư (peer thứ 4). Instance đầu: `wails dev` trong app/.
...
.DESCRIPTION
  ...
  Script này:
    - db: app/.local/dev-wails-peer4.db
    - p2p-port: 4004
...
.EXAMPLE
  .\scripts\dev-fourth-instance.ps1 -UseGoRun -NoNewWindow
#>
param(
    [string] $Bootstrap = '',
...
$db = [System.IO.Path]::GetFullPath((Join-Path $LocalInApp 'dev-wails-peer4.db'))
$port = 4004
...
if (-not $UseGoRun -and $AutoBuild) {
    Write-Host "Auto build: wails build (instance 4)" -ForegroundColor DarkGray
...
Write-Host "Fourth instance: db=$db  port=$port  headless=$Headless" -ForegroundColor Cyan
if ($Bootstrap -ne '') {
