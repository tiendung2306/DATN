...
| BI-046 | P0 | Invite legacy | Save pending invite từ welcome | Seed welcome hoặc fetch store mock | Refresh/list pending invites | Có pending invite với group_id/group_type/source |
| BI-047 | P0 | Invite legacy | Accept invite happy path | Có pending invite valid | `AcceptInvite(inviteID)` | Join group thành công, invite marked accepted/removed khỏi pending |
| BI-048 | P0 | Invite legacy | Reject invite | Có pending invite | `RejectInvite(inviteID)` | Invite marked rejected, group không join |
...
| BI-058 | P0 | Invite request | Request group invite happy path | Member requester, target peer | `RequestGroupInvite()` | Record pending, creator/requester/target đúng |
| BI-059 | P0 | Invite request | Request duplicate bị chặn/idempotent | Đã có pending request cùng target | Request lại | Không tạo duplicate active |
| BI-060 | P0 | Invite request | Target đã là member bị chặn | Target trong roster | `RequestGroupInvite()` | Lỗi already member |
...
| BI-063 | P0 | Invite request | Cancel request bởi requester | Requester có pending request | `CancelGroupInviteRequest()` | Status cancelled, creator thấy cập nhật khi sync |
