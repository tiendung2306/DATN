...
import { Hash, MessageSquare, Plus, Search, Check } from 'lucide-react'
import { runtimeClient } from '../../../services/runtime/runtimeClient'

interface CreateGroupModalProps {
  isOpen: boolean
  onClose: () => void
  onCreate: (name: string, type: 'channel' | 'dm', members: string[]) => Promise<void>
  creating: boolean
}
...
export default function CreateGroupModal({
  ...
  const [groupType, setGroupType] = useState<'channel' | 'dm'>('channel')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onCreate(trimmedName, groupType, members)
      setGroupName('')
      setMembers([])
      onClose()
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-5 mt-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={groupType === 'channel' ? 'e.g. thảo-luận-chung' : 'e.g. Nhóm Dự Án'}
          ...
          <div className="space-y-2">
            <Label className="text-xs text-slate-400 font-semibold uppercase tracking-wider flex justify-between items-center">
              <span>Mời thành viên ({members.length})</span>
            </Label>
            ...
            <div className="mt-2 border border-slate-800 bg-slate-950 rounded-lg max-h-48 overflow-y-auto divide-y divide-slate-800/50">
              {filteredPeers.length === 0 ? (
                <p className="text-xs text-slate-600 italic text-center py-4">Không tìm thấy người dùng nào</p>
              ) : (
