...
type BackupPayload struct {
	...
	Groups          []db.BackupGroupRecord    `json:"groups,omitempty"`
	StoredMessages  []db.BackupStoredMessage  `json:"stored_messages,omitempty"`
	KPBundles       []db.BackupKPBundle       `json:"kp_bundles,omitempty"`
	PendingWelcomes []db.BackupPendingWelcome `json:"pending_welcomes,omitempty"`
}
...
func ExportIdentityBackup(database *db.Database, privKey p2pCrypto.PrivKey, passphrase string) ([]byte, error) {
	if database == nil {
...
func ImportIdentityBackup(database *db.Database, encrypted []byte, passphrase string) (*BackupPayload, error) {
	if database == nil {
		return nil, fmt.Errorf("database is nil")
	}
	if passphrase == "" {
	...
	if err := database.SaveMLSIdentity(&db.MLSIdentity{
