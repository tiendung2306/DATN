...
func TestIdentityBackupRoundTrip(t *testing.T) {
	...
	if err := database.RestoreGroupsFromBackup([]db.BackupGroupRecord{
		{
			GroupID:    "g1",
	...
	if err := database.RestoreStoredMessagesFromBackup([]db.BackupStoredMessage{
		{
			GroupID:       "g1",
			Epoch:         7,
	...
	if err := database.RestoreKPBundlesFromBackup([]db.BackupKPBundle{
		{PeerID: peerID.String(), PublicKP: []byte("pk"), PrivateBundle: []byte("kb")},
	}); err != nil {
		t.Fatalf("RestoreKPBundlesFromBackup seed: %v", err)
	}
	if err := database.RestorePendingWelcomesFromBackup([]db.BackupPendingWelcome{
