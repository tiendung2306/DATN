...
// GrpcMLSEngine adapts the gRPC MLSCryptoServiceClient to the MLSEngine
// interface used by the Coordinator. All byte slices are passed through
// without interpretation — only the Rust engine deserializes them.
type GrpcMLSEngine struct {
	client mls_service.MLSCryptoServiceClient
}

// NewGrpcMLSEngine wraps an existing gRPC client connection.
func NewGrpcMLSEngine(client mls_service.MLSCryptoServiceClient) *GrpcMLSEngine {
...
func (g *GrpcMLSEngine) CreateProposal(ctx context.Context, groupState []byte, pType ProposalType, data []byte) (proposalBytes []byte, err error) {
